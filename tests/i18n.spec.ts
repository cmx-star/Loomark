...
describe('application locale', () => {
  ...
  it('keeps English as the explicit alternate locale and translates native menu labels', () => {
    ...
    expect(messages['zh-CN'].menu.openLogDirectory).toBe('打开日志目录')
    ...
    expect(messages['zh-CN'].status.progressive).toBe('渐进加载')
    expect(messages['en-US'].menu.file).toBe('File')
    expect(messages['en-US'].menu.openLogDirectory).toBe('Open Log Directory')
    ...
    expect(messages['en-US'].status.progressive).toBe('progressive load')
  })
})
