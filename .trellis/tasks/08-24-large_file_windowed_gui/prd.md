...
- [ ] 打开 ≤64MB 文件，行为与现状一致（完整编辑、预览正常），无新增降级提示。
- [ ] 打开一个 64~256MB 文件，出现大文件模式提示，且应用保持可响应（不因全文读入而长时间无响应）。
- [ ] 打开一个 256~512MB 文件，出现 EXTREME 档提示，编辑器进入受限模式且主界面不冻结。
- [ ] 尝试打开超过 512MB 的文件被拒绝并显示原因。
- [ ] 大文件打开后预览区基于 buildPreviewIndex 块索引渲染标题/段落/代码围栏内容，达到上限时明确显示截断状态。
- [ ] LARGE/EXTREME 档下编辑器侧内存占用不随文件大小线性增长到全文件规模（窗口化读取生效）。
- [ ] 耗时索引/预览构建期间 UI 不冻结（可交互或可取消）。
- [ ] 模拟磁盘空间不足时保存失败，原文件内容未被破坏，且有明确错误提示。
