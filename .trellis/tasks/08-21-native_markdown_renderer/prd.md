...
- 当前 GUI 使用 `QPlainTextEdit` 编辑、`QTextBrowser::setMarkdown()` 预览，并以 220 ms 防抖刷新。
...
- R3：主预览不得调用 `QTextBrowser::setMarkdown()`，不得生成 HTML 后交给 WebView，也不得新增 WebEngine、Node 或 JavaScript 运行时。
...
- R6：数学 AST 首版保留表达式内容并使用区别于正文的原生文本样式，不引入新的公式排版依赖。
- R7：保留现有 220 ms 防抖、完整/窗口化预览阈值、滚动同步提示和打开/编辑/保存行为。
- R8：README 记录新预览架构、固定依赖、首次配置需要联网、WebEngine 不在当前路线中，以及 Mermaid 为后续事项。
- R9：保留工作区现有未提交 GUI、主题和 Trellis 变更，不回退或重写无关内容。

## Acceptance Criteria

- [ ] CMake 首次配置能从官方仓库取得固定提交，后续构建链接 `md4qt::md4qt`，并关闭依赖自身测试、工具与安装目标。
- [ ] 常用 Markdown fixture 经新渲染模块生成非空 `QTextDocument`，标题、行内样式、代码块、列表、引用、表格、链接和分隔线具有可验证的原生文档结构或格式。
- [ ] 原始 HTML 不被当作富文本执行；数学表达式内容仍然可见。
- [ ] GUI 预览刷新不再调用 `setMarkdown()`，但小文档整篇预览和大文档动态截取行为保持。
- [ ] 桌面 GUI 仍能打开、编辑、预览并通过核心原子写入路径保存文件。
- [ ] README 明确说明原生 AST 渲染路线，并把 Mermaid 标为后续能力。
- [ ] `cmake --preset macos-debug`、`cmake --build --preset macos-debug`、`ctest --preset macos-debug` 通过。
- [ ] GUI 启动检查通过，预览区对代表性 Markdown 文档可见且无 WebEngine 进程或依赖。
